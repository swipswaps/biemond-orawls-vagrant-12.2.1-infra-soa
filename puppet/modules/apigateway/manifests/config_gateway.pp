# == Define: apigateway::config_gateway
#
# configs master gateway for OAG 11.2.3.4
#
##
define apigateway::config_gateway(
 ...
 
{
  ...
  exec { "Set up API gateway instance":
    command => "${oag_home_dir}/apigateway/posix/bin/managedomain --username admin --password changeme -c -n ${oag_instance_name} -g ${oag_group_name}",   
    path    => $exec_path,
    user    => $os_user,
    group   => $os_group,                             
  }
