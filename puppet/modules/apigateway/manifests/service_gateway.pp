# == Define: apigateway::service_gateway
#
# configures gateway as linux service for OAG 11.2.3.4
#
##
define apigateway::service_gateway(
 ...
 
{
  ...
  file { 'apigateway':
          ...
          source  => "${oag_download_dir}/apigateway",
                                 replace => true,
                                 path    => '/etc/init.d/apigateway',
                                 mode    => '0755',                         
  }
  
  file { 'apigateway-config':
          ensure  => present,
          source  => "${oag_download_dir}/apigateway-config-master",
                                 replace => true,
                                 path    => '/etc/default/apigateway',                    
                                 require => File["apigateway"],
  }
    
  service { 'apigateway':
    enable => true,
                ensure => 'running',
                require => File["apigateway-config"],
  }
